...
def pp_plot(ax, quantiles, p_array, double=False):
    ...
    ax.set_xlabel("posterior quantile $p$", fontsize=20)
...
fig, ax = plt.subplots(8, 1, figsize = (8, 40))
fig.subplots_adjust(hspace = 0.4, wspace = 0.1, top = 0.98, bottom = 0.05, left = 0.1, right = 0.98)
p_array = np.linspace(0, 1, 50)
for j, cax in enumerate(ax):
    pp_plot(cax, quantiles[:, j], p_array, double=False)
    cax.text(0.1, 0.8, parameter_names[j], fontsize=20)
