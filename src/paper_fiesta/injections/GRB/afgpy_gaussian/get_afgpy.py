...
posterior_nmma = pd.read_csv("./outdir_nmma/injection_gaussian_posterior_samples.dat", sep=" ")
best_ind = np.argmax(posterior_nmma["log_likelihood"])

parameters = dict(posterior_nmma.iloc[best_ind])

_, mags = model.generate_lightcurve(np.logspace(np.log10(1e-2), np.log10(200), 100), parameters)

for key, val in mags.items():
    mags[key] = val + 5*np.log10(40e6) - 5

mags = pd.DataFrame(mags)

mags.to_csv("./figures/nmma_afgpy_best_fit_mags.dat", sep= " ")